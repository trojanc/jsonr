...
func Unmarshal(data []byte, opts ...UnmarshalOption) (any, error) {
	...
	if valueOk && typeKeyOk {

		if value == nil {
			return value, nil
		}

		return deserializeObject(value.(map[string]any), typeKey.(string), options)
	}
...
func deserializeObject(data map[string]any, instanceType string, opts *unmarshalOptions) (any, error) {
	isPointer := false
	isSlice := false
	isMap := false
	mapKeyType := ""
